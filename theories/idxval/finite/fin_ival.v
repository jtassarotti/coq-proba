...
Require Import Reals Psatz Logic.EqdepFacts.
...
Definition le_ival {X} (I1 I2: ival X) :=
  ∃ h1: (support (val I2) → support (val I1)),
    (∀ b, ∃ a, h1 a = b) ∧
    (∀ a, ind I1 (sval (h1 a)) = ind I2 (sval a)) ∧
    (∀ b, \big[Rplus/0]_(i | h1 i == b) val I2 (sval i) = val I1 (sval b)).
...
Lemma le_ival_refl {X} (I: ival X): le_ival I I.
Proof.
  exists (λ x, x).
  repeat split; try done.
  - eexists; eauto.
  - intros b. by rewrite big_pred1_eq.
Qed.
...
Lemma eq_ival_quasi_refl {X} (it: finType) (i: it → X) v pf1 pf2:
...
Lemma le_ival_quasi_refl {X} (it: finType) (i: it → X) v pf1 pf2:
  le_ival {| idx := it; ind := i; val := v; val_nonneg := pf1 |}
          {| idx := it; ind := i; val := v; val_nonneg := pf2 |}.
Proof.
  exists (λ x, x).
  repeat split; try done.
...
Global Instance le_ival_Reflexive {X}: Reflexive (@le_ival X).
Proof. intros ?. apply le_ival_refl. Qed.

Lemma le_ival_eq {Y} (I1 I1' I2 I2': ival Y):
  eq_ival I1 I1' →
  eq_ival I2 I2' →
  le_ival I1 I2 →
  le_ival I1' I2'.
Proof.
  intros Heq1 Heq2 Hle.
  destruct Hle as (hle&Hsurj&Hind&Hval).
  destruct Heq1 as (h1&h1'&Hinv1&Hinv1'&Hind1&Hval1).
  symmetry in Heq2.
  destruct Heq2 as (h2'&h2&Hinv2'&Hinv2&Hind2'&Hval2').
  exists (λ x, h1 (hle (h2' x))).
  repeat split.
  * intros i1'. edestruct (Hsurj (h1' i1')) as (i2&Heq).
    exists (h2 i2).
    rewrite Hinv2. congruence.
  * intros i2'. by rewrite Hind1 Hind Hind2'.
  * rewrite //=. intros i1'.
    etransitivity.
    apply (eq_bigl (λ x, hle (h2' x) == h1' i1')).
    { intros x => //=. apply /eqP. case: ifP.
      * move /eqP => ->. by rewrite Hinv1'.
      * move /eqP => Hneq Heq. apply Hneq.
        apply (f_equal h1') in Heq. rewrite Hinv1 in Heq. done.
    }
    rewrite -{2}(Hinv1' i1').
    rewrite Hval1 -Hval.
    eapply sum_reidx_surj1 => //=.
    ** intros i2' ?. move /eqP => ->; split.
        *** by rewrite /index_enum -enumT mem_enum.
        *** by rewrite eqxx.
    ** intros i2 ?. move /eqP => Heq.
       exists (h2 i2); split.
        *** by rewrite /index_enum -enumT mem_enum.
        *** rewrite Hinv2 Heq eqxx. done.
    ** rewrite /index_enum -enumT. apply enum_uniq.
    ** rewrite /index_enum -enumT. apply enum_uniq.
    ** intros i2 i2' Heq%(f_equal h2). 
       by rewrite ?Hinv2' in Heq. 
Qed.

Global Instance le_ival_proper : Proper (@eq_ival X ==> @eq_ival X ==> iff) (@le_ival X).
Proof.
  intros I1 I1' Heq1 I2 I2' Heq2.
  split; intros; eapply le_ival_eq; eauto; try symmetry; eauto.
Qed.
...
refine(
  λ A B f I,
  {| idx := [finType of { i: idx I & idx (f (ind I i)) }];
...
Proof.
  intros HP.
  refine {| idx := [finType of {x: idx m | Rgt_dec (val m x) 0}]; val := (λ x, val m (sval x));
            val_nonneg := _;
            ind := _ |}.
  - intros (im&Hgt).
    exists (ind m im).
...
Proof. 
  ...
  - intros (im1&if1) Hgt => //=.
    ...
    specialize (Hfval2 _ if2) => //=.
    assert (eq_dep _ (λ Ty, option (idx (f1 Ty))) _
  ...
  - intros (im2&if2) Hgt => //=.
    ...
    specialize (Hfval1 _ if1) => //=.
    assert (eq_dep _ (λ Ty, option (idx (f2 Ty))) _
  ...
  - intros (im1&if1) (Hgt1&Hgt2)%val_bind_gt.
    ...
    rewrite //=.
    f_equal.
    eapply f_eq_dep_non_dep with (f := λ p x, ind (f2 p) x).
  ...
  - intros (im1&if1) (Hgt1&Hgt2)%val_bind_gt.
    ...
    rewrite //=.
    f_equal.
    eapply f_eq_dep_non_dep with (f := λ p x, val (f2 p) x).
    generalize (Hind1x r). intros Heq. inversion Heq as [Heq'].
    rewrite /eq_rect_r.
    symmetry in Heq'.
    assert (Heq' = (Logic.eq_sym (ival_bind_congr_subproof A m1 m2 im1 im2 Heq))) as <-.
...
Lemma empty_idx_zero_ival {X: Type} (I: ival X):
  Finite.enum (idx I) = [::] →
  eq_ival I zero_ival.
Proof.
  intros Heq. symmetry. apply eq_ival_nondep_suffice.
  assert (I_rect: ∀ (P: idx I → Type) (e: idx I), P e).
  { 
    intros P e.
    assert (e \in [::]) as Hin; last by (rewrite //=).
    rewrite -Heq. rewrite -enumT mem_enum //. 
  }
  exists (λ x, Empty_set_rect _ x).
  exists (λ x, I_rect _ x).
  repeat split => //=; auto.
  - intros b. destruct (I_rect _ _). 
  - intros b. destruct (I_rect _ _). 
Qed.
