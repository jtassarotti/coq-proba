...
From discprob.monad.finite Require Import monad monad_cost_hoare monad_cost quicksort quicksort_correct.
From discprob.monad.finite Require Import max_triangle.
...
Proof.
  move /permP => Hcount.
  apply /permP => P'. rewrite ?count_filter. apply Hcount.
Qed.
...
Proof.
  ...
  { apply perm_size => //. }
  move /perm_iotaP in Hperm. specialize (Hperm a).
  destruct Hperm as [Is Hperm Heq].
  assert (HsizeIs: size Is = size l2) by (rewrite (perm_size Hperm) size_iota //).
    ...
    * intros (x, Hle) Hin.
      ...
      ** rewrite -ldist_assoc. rewrite Ex_mbind_mret.
         ...
         *** rewrite //= in Heq. rewrite -Heq.
             ...
             rewrite -(perm_size (perm_filter _ _ Hperm)).
